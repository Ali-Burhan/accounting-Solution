...
const Homebankpayments = () => {
    ...
    const filterContent = (e) => {
        ...
        const newUsers = bankpayment.filter((user) => user.employeeid == searchValue);
      ...
      const handleBankPayments = async () => {
        let headersList = {
           ...
           let response = await fetch("/getbankpayments", { 
             method: "GET",
             headers: headersList
           });
...
const handleedit = (ids) => {      
  ...
  return (
    ...
    <input type="text" name="search" className='mx-2 usersearch'  placeholder='Search with ID' id="search1" onChange={filterContent} />
            <Link to={'/bankpayment'} className='btn btn-outline-primary'>New Bank Payment</Link>
        </div> 
        <div className="">
        </div>
...
export default Homebankpayments