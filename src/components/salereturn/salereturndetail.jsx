...
const Customerreturninvoicedetail = () => {
	...
	const handlepayment = async () =>{
		let headersList = {
		   ...
		   let bodyContent = JSON.stringify({
			...
			paid,
			selectedproduct,
			supplierid: supplierinvoice.length > 0? supplierinvoice[0].customerid:0,
			bankid: paid?accountactivityidselected:0
		   });
		   
		   let response = await fetch("/addsalereturninvoice", { 
			 method: "POST",
			 body: bodyContent,
			 headers: headersList
		   });
	...
	useEffect(()=>{
		setsubtotalamount(supplierinvoicedetail.length > 0? quantities.length>0? quantities.map((row)=>(row==0? supplierinvoicedetail[0].saleunitprice*0:supplierinvoicedetail[0].saleunitprice*row)).reduce((pre,cur)=>(pre+cur),0):0:"")
	},[quantities])
	useEffect(()=>{
...
{ paid &&
    ...
    placeholder={
      returninvoicedetail.length > 0
        ? returninvoicedetail
            ...
            .map(ele =>
              ele.returnquantity === 0 ? "nothing" : row.salequantity - ele.returnquantity
            )
  ...
  />
    								...
    								<td class="text-right">{quantities.length>0? quantities[index]?  quantities[index] * row.saleunitprice: 0 * row.saleunitprice:0}</td>
    							</tr>
									)) :""
								}	
    							<tr>
...
export default Customerreturninvoicedetail