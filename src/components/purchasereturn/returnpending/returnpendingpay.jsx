...
const Returnpendingpay = () => {
    ...
    const getPendingInvoices = async () => {
        ...
        let headersList = {
           ...
           let response = await fetch("/getpendingreturns", { 
             method: "GET",
             headers: headersList
           });
    ...
    const handleedit = (ids) => {      
      navigate(`/payreturnpendinginvoice/${ids}`);
      }
  return (
    <>
    <div className="table">
  ...
  <path d="M15.75 19.875V15h-15v1.5c0 2.37.27 3.357.678 4.108.69 1.273 1.94 1.892 3.822 1.892h12s-1.5-.938-1.5-2.625Z"></path>
                    ...
                    <TableCell align="right">{row.tax}</TableCell>
                     <TableCell align="right"><button className='btn2' onClick={()=>handleedit(row.supplierreturninvoiceid)}>Pay</button></TableCell>
                  </TableRow>
                ))
                }
              </TableBody>
...
export default Returnpendingpay